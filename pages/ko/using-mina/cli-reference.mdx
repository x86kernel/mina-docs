...
미나 CLI(명령줄 인터페이스)는 사용자가 미나 네트워크와 상호 작용하는 기본 방법입니다. 계정 생성, 트랜잭션 전송, 합의 참여를 위한 표준 클라이언트 기능을 제공합니다. 고급 사용자를 위한 고급 클라이언트 및 데몬 명령도 있습니다

CLI 는 <DocLink copy="미나 설치" url="/getting-started#installation" />시에 같이 설치됩니다.

<Alert kind="warning">

미나 API는 아직 개발 중이므로 아래의 명령들이 변경될 수 있습니다. 최신 버전은 `mina help` 를 참조하세요.
